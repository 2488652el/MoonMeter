...
export interface ClaudeKeyDetection {
   ...
   *  脱敏密钥,如 "sk-ant-...wXYZ",绝不返回完整密钥。 */
  maskedKey?: string
  /** File path the key was read from, or 'env:ANTHROPIC_API_KEY'.
   *  密钥来源路径,或 'env:ANTHROPIC_API_KEY'。 */
  path?: string
  /**
   ...
   * 渲染层只能看到 maskedKey 与 path,严禁记录日志、写入磁盘或跨信任边界透传。
   */
  fullKey?: string
}
 ...
 *
 * 从本机 Claude Code 安装中检测现有 Anthropic API 密钥;先查环境变量(最快、无需读文件),再查凭据文件。
 */
 ...
 *
 * 从本机 Codex CLI 安装中检测现有 OpenAI API 密钥;先查环境变量,再查 ~/.codex/auth.json。
 */
 ...
 *
 * 一次性检测 Claude 与 Codex 密钥,供"导入现有密钥"UI 流程使用。
 */
